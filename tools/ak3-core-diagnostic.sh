...
split_boot() {
  ...
  if [ -f "$BIN/nanddump" ]; then
    ui_print "DD_START: reading $BLOCK -> $BOOTIMG";
    nanddump -f $BOOTIMG $BLOCK;
    ui_print "DD_DONE (nanddump)";
  else
  ...
  if [ -f "$BIN/unpackelf" ] && unpackelf -i $BOOTIMG -h -q 2>/dev/null; then
    ...
    ui_print "MB_START: magiskboot unpack -h $BOOTIMG";
...
flash_boot() {
  ...
  if [ -f "$BIN/mkimage" ]; then
    ...
    ui_print "REPACK_START: magiskboot repack $BOOTIMG";
    magiskboot repack $nocompflag $BOOTIMG $AKHOME/boot-new.img;
    ui_print "REPACK_DONE: $?";
  fi;
  if [ $? != 0 ]; then
  ...
  if [ -f "$BIN/flash_erase" -a -f "$BIN/nandwrite" ]; then
    ...
    ui_print "WRITE_START: zero-fill $BLOCK";
    dd if=/dev/zero of=$BLOCK $CUSTOMDD 2>/dev/null;
    ui_print "WRITE_ZERO_DONE";
