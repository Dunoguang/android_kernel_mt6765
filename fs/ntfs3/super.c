...
static int ntfs3_parse_options(char *data, struct ntfs_mount_options *opts,
			       unsigned long *sb_flags)
{
	...
	while ((opt = strsep(&data, ",")) != NULL) {
		...
		if (!strcmp(opt, "uid")) {
			...
			if (!value || kstrtou32(value, 0, &v))
				goto invalid;
			opts->fs_uid = make_kuid(current_user_ns(), v);
			...
			if (!value || kstrtou32(value, 0, &v))
				goto invalid;
			opts->fs_gid = make_kgid(current_user_ns(), v);
			if (!gid_valid(opts->fs_gid))
				goto invalid;
		} else if (!strcmp(opt, "umask")) {
			u32 v;

			if (!value || kstrtou32(value, 8, &v) || (v & ~07777))
				goto invalid;
			opts->fs_fmask_inv = ~v;
			opts->fs_dmask_inv = ~v;
			opts->fmask = 1;
			...
			opts->fs_fmask_inv = ~v;
			opts->fmask = 1;
		} else if (!strcmp(opt, "sys_immutable")) {
			opts->sys_immutable = negated ? 0 : 1;
		} else if (!strcmp(opt, "discard")) {
			opts->discard = negated ? 0 : 1;
		} else if (!strcmp(opt, "force")) {
			opts->force = negated ? 0 : 1;
		} else if (!strcmp(opt, "sparse")) {
			opts->sparse = negated ? 0 : 1;
		} else if (!strcmp(opt, "hidden")) {
			opts->nohidden = negated ? 1 : 0;
		} else if (!strcmp(opt, "acl")) {
			if (!negated) {
#ifdef CONFIG_NTFS3_FS_POSIX_ACL
				*sb_flags |= SB_POSIXACL;
#else
				pr_err("ntfs3: Support for ACL not compiled in!\n");
				return -EINVAL;
#endif
			} else {
				*sb_flags &= ~SB_POSIXACL;
			}
		} else if (!strcmp(opt, "showmeta")) {
			opts->showmeta = negated ? 0 : 1;
		} else if (!strcmp(opt, "iocharset")) {
			if (!value)
				goto invalid;
			kfree(opts->nls_name);
			opts->nls_name = kstrdup(value, GFP_NOFS);
			if (!opts->nls_name)
				return -ENOMEM;
		} else if (!strcmp(opt, "prealloc")) {
			opts->prealloc = negated ? 0 : 1;
		} else if (!strcmp(opt, "acsrules")) {
			opts->noacsrules = negated ? 1 : 0;
		} else {
			goto invalid;
		}
	}
	return 0;
invalid:
	pr_err("ntfs3: Invalid value for %s.\n", opt);
	return -EINVAL;
}
...
{
	struct ntfs_sb_info *sbi = sb->s_fs_info;
	struct ntfs_mount_options *new_opts;
	unsigned long mnt_flags = 0;
	int ro_rw, err = 0;

	new_opts = kzalloc(sizeof(*new_opts), GFP_NOFS);
	if (!new_opts)
		return -ENOMEM;

	new_opts->fs_uid = current_uid();
	new_opts->fs_gid = current_gid();
	new_opts->fs_fmask_inv = ~current_umask();
	new_opts->fs_dmask_inv = ~current_umask();

	err = ntfs3_parse_options(data, new_opts, &mnt_flags);
	if (err)
		goto out;
	...
	*flags |= (int)mnt_flags;
	...
	if (IS_ERR(new_opts->nls)) {
		new_opts->nls = NULL;
		ntfs_err(sb, "ntfs3: Cannot load iocharset %s",
			 new_opts->nls_name);
		err = -EINVAL;
		goto out;
	}
	if (new_opts->nls != sbi->options->nls) {
		ntfs_err(sb, "ntfs3: Cannot use different iocharset when remounting!\n");
		err = -EINVAL;
		goto out;
	}
	...
	if (ro_rw && (sbi->volume.flags & VOLUME_FLAG_DIRTY) &&
	    !new_opts->force) {
		ntfs_err(sb, "ntfs3: Volume is dirty and \"force\" flag is not set!\n");
		err = -EINVAL;
		goto out;
	}

	memcpy(sbi->options, new_opts, sizeof(*new_opts));
	put_mount_options(new_opts);
	return 0;
out:
	put_mount_options(new_opts);
	return err;
...
static int ntfs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	err = ntfs3_parse_options(data, opts, &sb->s_flags);
	if (err)
		goto out;
	...
	if (inode->i_size < ntfs_bitmap_size(tt)) {
		err = -EINVAL;
		goto put_inode_out;
	}
